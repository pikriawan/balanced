...
export async function createJournal(companyId, formData, type) {
    const session = await auth();
    ...
    redirect(`/companies/${companyId}/journals/${type}`);
}

export async function editJournal(companyId, journalId, formData, type) {
    const session = await auth();
