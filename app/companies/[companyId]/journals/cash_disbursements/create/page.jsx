...
export default async function CashDisbursementsournalCreatePage({ params }) {
    const { companyId } = await params;
    const accounts = await getAccounts(companyId);
    const lastJournalNumber = await getLastJournalNumber(companyId, "KK", "cash_disbursements");
    ...
    return (
        <div className="p-4 flex flex-col items-start gap-4">
            ...
            <SpecialJournalCreateForm companyId={companyId} accounts={accounts} lastJournalNumber={lastJournalNumber} type="cash_disbursements" />
        </div>
    );
}
