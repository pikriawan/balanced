...
export default async function CashReceiptsJournalEditPage({ params }) {
    const { companyId, journalId } = await params;
    const accounts = await getAccounts(companyId);
    const journal = await getJournal(companyId, journalId, "cash_receipts");
    ...
    return (
        <div className="p-4 flex flex-col items-start gap-4">
            ...
            <SpecialJournalEditForm companyId={companyId} accounts={accounts} journal={journal} type="cash_receipts" />
        </div>
    );
}
